...
def build_sources_from_urls(input_urls: list[str], metadata: list[SourceMetadata]) -> list[Source]:
  """Build source metadata directly from the provided input URLs."""
  sources: list[Source] = []
  for index, raw_url in enumerate(input_urls):
    url: str = raw_url.strip()
    if not url:
      raise ValueError("-i values cannot be empty")
    source_id: str = f"{metadata[index].source_name}".strip().replace(" ", "_").replace(":", "-").replace("/", "_")
    sources.append(Source(source_id=source_id, url=url))
  return sources
...
async def run_extraction(
  ...
  for source, return_code in results:
    if return_code == 0:
      print(f"[OK] Source '{source.source_id}' finished successfully")

  if failures:
    for source, return_code in failures:
      print(
        f"[ERROR] Source '{source.source_id}' failed with code {return_code}",
        file=sys.stderr,
      )
    return 1
